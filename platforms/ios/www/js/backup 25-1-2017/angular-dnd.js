...
module.directive('dndResizable', ['$parse', '$timeout', function($parse, $timeout) {
    ...
    function createHandleElement(side) {
        if (side=="se") {
            return angular.element('<div><img src="img/resize2.png" /></div>').addClass('angular-dnd-resizable-handle angular-dnd-resizable-handle-' + side);
        }else {
    ...
    function link (scope, $el, attrs, ctrls) {
        ...
        function getBindings(side) {
            ...
            function drag(api) {
                ...
                // It resizes from all the directions correctly
                // styles.height = height * scale.y;

                // It resizes but also keeps the acpect ratio of the div
                styles.height = height * scale.x;
